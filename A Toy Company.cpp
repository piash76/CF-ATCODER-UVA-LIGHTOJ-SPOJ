...
ll bfs(string &src,string &des)
{
   ...
   while(!q.empty())
   {
       ...
       for(ll i=0;i<3;i++)
       {
           string child1=prnt,child2=prnt;
           if(child1[i]<'z') child1[i]++;  ///+
           else child1[i]='a';

           if(child2[i]>'a') child2[i]--; ///-
           else child2[i]='z';

           if(vis[child1]==0 and m[child1]!=1)
           {
               vis[child1]=1;
               level[child1]=level[prnt]+1;
               if(child1==des) return level[child1];
               q.push(child1);
           }

           if(vis[child2]==0 and m[child2]!=1)
           {
               vis[child2]=1;
               level[child2]=level[prnt]+1;
               if(child2==des) return level[child2];
               q.push(child2);
           }
       }
