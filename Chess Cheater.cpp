...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i=0;i<n;i++) if(s[i]=='W') cw++;
        for(i=0;i<n;i++) if(s[i]=='W') {l=i;break;}
        for(i=n-1;i>=0;i--) if(s[i]=='W') {r=i;break;}


        if(cw+k>=n) cout<<max(0ll,2*n-1)<<endl;
        else if(l==-1 and r==-1)
        {
        ...
        else
        {
          ...
          //  for(auto x:v1) cout<<"x "<<x<<" ";
            ...
            for(i=0;i<l;i++) v2.push_back(1);   ///for the LLL..WWLLWW...LLLL
            for(i=r+1;i<n;i++) v2.push_back(1);
