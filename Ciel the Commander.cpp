...
///https://www.youtube.com/watch?v=HslIHMbL7zQ
...
ll dfs_for_sub(ll node,ll parent)
{
    sub[node]=1;
    ...
    for(auto child : g[node])
    {
        ...
        sub[node]+=dfs_for_sub(child,node);
    }
    return sub[node];

}

ll dfs_for_centroid(ll node,ll parent,ll n)
{
    for(auto child:g[node])
    {
        if(child==parent) continue;
...
void decompose(ll node,ll parent,char ranking)
{
   ll n=dfs_for_sub(node,-1); ///total len  n  of tree rooted at given node
   ll centroid=dfs_for_centroid(node,parent,n);  ///centroid for tree roooted in given node

   par[centroid]=parent; ///for  centroid nodes parent is this parent
   ans[centroid]=ranking;

   for(auto child:g[centroid])
   {
       g[child].erase(centroid);
       decompose(child,centroid,ranking+1);
   }
