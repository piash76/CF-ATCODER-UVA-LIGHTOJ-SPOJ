...
///
const int maxn = 2111111;  ///2e5


ll a[maxn],b[maxn],pref[maxn],suf[maxn];
ll n,m;

ll getmin(ll val)
{
    ll pos=lower_bound(b+1,b+m+1,val)-(b+1);
    ll s;
    //cout<<"POS "<<pos<<endl;
    if(pos)  s=abs(b[pos]-val);
    //cout<<"S1 "<<s<<endl;

    if(pos+1<=m)
    {
        s=min(s,abs(b[pos+1]-val));
    }
    //cout<<"S "<<s<<endl;
...
int main()
{
   ...
   // for(i=1;i<=m;i++) cout<<b[i]<<" ";
    ...
    for(i=1;i<=n;i++)
    {
       if(i%2==0)  pref[i]=pref[i-2]+abs(a[i]-a[i-1]);
    }

    for(i=n;i>=1;i--)
    {
        if(i%2==0) suf[i]=suf[i+2]+abs(a[i]-a[i+1]);
    }
    ...
    {
      //  cout<<"A[i] "<<a[i]<<endl;
       // cout<<"MIN "<<getmin(a[i])<<endl;
        if(i%2==0)
        {
            ll x=abs(a[i-1]-a[i+1])+pref[i-2]+suf[i+2]+getmin(a[i]);
            ans=min(ans,x);
            //cout<<"X "<<x<<endl;
        }
        else
        {
            ll x1=pref[i-1]+suf[i+1]+getmin(a[i]);
            //cout<<"X1 "<<x1<<endl;
            ans=min(ans,x1);
        }
    }
