...
int main()
{
    ...
    ///first to swap something with zero

    for(i=0;i<n and tw>x;i++)
    {
        if(zr>=x) break;
        if(s[i]=='2')
        {
    ...
    ///to swap and use 1
    for(i=0;i<n and tw>x;i++) ///with first 2s
    {
        //cy;
        if(on>=x) break;
        if(s[i]=='2')
    ...
    for(i=n-1;i>=0 and zr>x;i--) ///with  last 0s
    {
        if(on>=x) break;
        if(s[i]=='0')
        {
