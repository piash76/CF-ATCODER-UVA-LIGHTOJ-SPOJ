...
int main()
{
    ...
    f2(i,1,n)
    {
        cin>>a[i];
        bit=a[i];
       /// bit.flip(1);  ///flips the bit at pos
        cout<<bit<<endl;
        ///cout<<bit.set()<<endl;   ///sets all bits

        ///cout<<bit.reset()<<endl;   ///resets all bits

        ///cout<<bit.size()<<endl;

        ///cout<<bit.any()<<endl;  ///any function returns true, if atleast 1 bit

        ///cout<<bit.count()<<endl; ///count function returns number of set bits in bitset
      //  for(j=0;j<3;j++) cout<<bit[j]<<" ";
       // cout<<endl;
    }
