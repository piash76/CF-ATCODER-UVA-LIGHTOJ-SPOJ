...
void dfs(ll node,ll par)
{
    ...
    for(auto child: g[node])
    {
        ...
        {
            // cout<<"BACK EDGE  NODE "<<node<<" CHILD "<<child<<endl;
            low[node]=min(low[node],in[child]); ///connecting the child with grand parent
            if(in[node]>in[child])  path.push_back({node,child});  ///connecting node to ancestor
        ...
        else
        {
           dfs(child,node);
            ...
            // cout<<"FRWARD EDGE  NODE "<<node<<" CHILD "<<child<<endl;

          // cout<<"NODE IN "<<in[node]<<" LOW "<<low[node]<<endl;
           //cout<<"CHILD IN "<<in[child]<<" LOW "<<low[child]<<endl;
