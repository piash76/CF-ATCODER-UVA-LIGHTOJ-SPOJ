...
int main()
{
    ...
    ///sum[R]-sum[L-1]=k; sum[R]=sum[L-1]+k;
    for(i=0;i<n;i++)
    {
        ///here i representing the sum[L-1];i==L;
        ll key=sum[i-1]+k; ///i as starting of substring

       ///how many right boundaries for sum[L-1]

        ll x=upper_bound(sum+i,sum+n,key)-sum;
        ll y=lower_bound(sum+i,sum+n,key)-sum;
