...
//from a loser
//ll a[maxn];
ll dp[105][105],n,w,k,dust[105];

ll calc(ll idx,ll moves)
{
    if(idx>=n or moves==0) return 0;
    if(dp[idx][moves]!=-1) return dp[idx][moves];

    ll x=0,i;
    ll ep=dust[idx]+w;
    for(i=idx;i<n and dust[i]<=ep;i++)
    {
        x++;
    }

    ll ans1=x+calc(i,moves-1);
    ll ans2=calc(idx+1,moves);

    dp[idx][moves]=max(ans1,ans2);
    return dp[idx][moves];
